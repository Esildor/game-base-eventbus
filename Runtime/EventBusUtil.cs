...
namespace GameBase.EventBus
{
    ...
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(EventBusUtil));
        ...
        static List<Type> InitializeAllBuses()
        {
            ...
            foreach (var eventType in EventTypes)
            {
                ...
                log.DebugFormat("Initialized EventBus<{0}>", eventType.Name);
            }
        ...
        public static void ClearAllBuses()
        {
            if (EventBusTypes == null)
                return; //TODO: Double check if this is a correct state (happens within projects with dynamic reloading)
            ...
            try
            {
                for (int i = 0; i < EventBusTypes.Count; i++)
                {
                    ...
                    if (log.IsDebugEnabled)
                        sb.AppendLine(busType.GenericTypeArguments[0].Name);
                }
            }
            catch (Exception e) { log.Error("Failed to clear all event buses", e); }

            log.DebugFormat("Cleared <b>{0}</b> buses.\n{1}", EventBusTypes.Count, sb);
        }
    }
}